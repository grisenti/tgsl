...
pub const MAX_STACK: usize = MAX_CALLS * MAX_LOCALS;
  ...
  pub fn interpret(
    ...
    loop {
      ...
      match op {
        ...
        OpCode::MakeClosure => make_closure(&mut frame, functions, &mut self.gc),
        OpCode::GetGlobal => {
        ...
        OpCode::Return => {
          if function_call == 0 {
            ...
            let ret = frame.pop();
            function_call -= 1;
            frame = self.call_stack[function_call];
            frame.push(ret)?;
          }
        }
